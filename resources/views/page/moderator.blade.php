                    ...
                    <div class="flex overflow-x-auto  gap-2 sm:gap-3 p-2 sm:p-4 
                        ...
                        <a href="{{ route('moderator.index') }}" class="flex-shrink-0">
                            <button class="px-3 sm:px-4 py-2 rounded-full border shadow-sm text-xs sm:text-sm
                                ...
                                {{ request()->routeIs('moderator.index') 
                        ...
                        @foreach ($kategoris as $kategori)
                            <a href="{{ route('moderator.kategori', $kategori->id) }}" class="flex-shrink-0">
                                <button class="px-3 sm:px-4 py-2 rounded-full border shadow-sm text-xs sm:text-sm
                                    whitespace-nowrap transition-all duration-200 hover:shadow-md
                                    {{ request()->is('moderator/kategori/' . $kategori->id) 
                                        ? 'bg-blue-500 text-white border-blue-500' 
                                        : 'bg-gray-100 text-gray-700 border-gray-300 hover:border-blue-300 hover:bg-gray-50' }}">
                                    {{ $kategori->nama }}
                                </button>
                ...
                @empty
                    <div class="col-span-full flex justify-center items-center py-16 sm:py-20 px-4">
                        <div class="text-center text-gray-500 space-y-4 max-w-md">
                            <i class="fas fa-user-cog text-3xl sm:text-4xl md:text-5xl text-gray-400"></i>
                            <div class="space-y-2">
                                <p class="text-base sm:text-lg font-medium">Belum ada data moderator</p>
            ...
            @if(count($moderators) > 0)
                <div class="mt-8 sm:mt-12 text-center">
                    <p class="text-sm text-gray-500">
                        Menampilkan {{ count($moderators) }} moderator
                        @if(request('q'))
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('focus', function() {
            if (this.value === '') {
                this.placeholder = 'Ketik minimal 2 karakter...';
            }
        });

        searchInput.addEventListener('blur', function() {
            this.placeholder = 'Cari Moderator...';
        });
    }
